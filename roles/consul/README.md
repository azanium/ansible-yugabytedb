# Note for Tokopedia maintainer

Some changes to this role:
- Added CAP_NET_BIND_SERVICE=+eip to consul binary when consul is using port 53
- Automatically disable systemd-resolved when using port 53 and ansible detected systemd service manager
- Use `item.id` and defaults to `item.name` for consul_services
- Changed consul_systemd_restart_sec defaults from 42s to 15s

# Below are the official README.md
